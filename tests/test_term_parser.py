...
from oyez_sa_asr import term_parser
...
class TestExpandAbbreviation:
    ...
    def test_expanded_two_digit_out_of_range_hits_line_57(self) -> None:
        """2-digit expansion out of range raises (term_parser line 57)."""
        with (
            patch.object(term_parser, "_MAX_YEAR", 2049),
            pytest.raises(ValueError, match="out of valid range"),
        ):
            expand_abbreviation("50")  # 2000+50=2050 > 2049
    ...
    def test_cross_century_both_1900s_end_less_than_start(self) -> None:
        """Cross-century when both in 1900s and end < start (line 100)."""
        # 99-98 -> 1999, 1998; both < 2000 so end_year becomes 2000+98=2098
        result = parse_range("99-98")
        assert "1999" in result
    ...
    def test_range_no_valid_years_after_filtering(self) -> None:
        ...
        with (
            patch.object(
                term_parser, "expand_abbreviation", side_effect=["2101", "2102"]
            ),
            patch.object(term_parser, "_MAX_YEAR", 2100),
            pytest.raises(ValueError, match="No valid years in range"),
        ):
            parse_range("x-y")
    ...
    def test_range_filters_out_of_range_years_continue(self) -> None:
        ...
        # Mock so range is 2099-2101; 2101 > _MAX_YEAR so skipped in loop (line 114)
        with patch.object(
            term_parser, "expand_abbreviation", side_effect=["2099", "2101"]
        ):
            result = parse_range("2099-2100")
        assert result == ["2099", "2100"]
        assert "2101" not in result
