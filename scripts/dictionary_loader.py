# Edited by Cursor (cascade: enchant → enchant alternate → WordNet)
"""Load English word checker for concatenated-word split and non-dictionary awareness.

Cascade: (1) enchant en_US (aspell, hunspell, nuspell if plugin present), (2) enchant la (Latin, if installed),
...
# Strings shorter than this are not looked up in WordNet (speed: skip expensive checks). Edited by Cursor.
_WORDNET_MIN_LEN = 4
...
def _get_wordnet() -> object:
    """Load WordNet once and return the corpus; reused in _wordnet_check. Edited by Cursor."""
...
def set_allow_no_enchant(allow: bool) -> None:
    """If True, allow fallback to word list when enchant is unavailable. Edited by Cursor."""
    global _ALLOW_NO_ENCHANT  # noqa: PLW0603
    _ALLOW_NO_ENCHANT = allow


def set_legal_dict_path_for_testing(path: Path | None) -> None:
    """Override path to legal words file for tests. Pass None to reset. Edited by Cursor."""
    global _legal_dict_path_override  # noqa: PLW0603
    _legal_dict_path_override = path


def _get_legal_dict_path() -> Path:
    """Path to legal words file (override for testing or default). Edited by Cursor."""
...
def _load_legal_set() -> frozenset[str]:
    """Load legal words from file; one word per line, lowercase, skip comments/empty. Edited by Cursor."""
...
class _EnchantChecker:
    """Container that supports __contains__(word) via enchant spell check; caches results. Edited by Cursor."""
...
def _get_stemmer() -> object:
    """Return lazy-loaded SnowballStemmer for English. Edited by Cursor."""
...
def _word_candidates(w: str) -> set[str]:
    """Return w plus morphy bases, stem, and suffix/prefix-derived forms. Edited by Cursor."""
    out: set[str] = set()
    w_lower = w.lower()
    if w_lower:
        out.add(w_lower)
    ...
    if w_lower.startswith("pre") and len(w_lower) > 5:
        ...
        if len(c) >= _WORDNET_MIN_LEN:
            out.add(c)
    # -ity/-ability, -ily/-ly, -ment, -ency (new awareness forms). Edited by Cursor.
    if w_lower.endswith("rability") and len(w_lower) > 8:
        c = w_lower[:-8] + "er"  # "rability" is 8 chars -> arbiter
        if len(c) >= _WORDNET_MIN_LEN:
            out.add(c)
    if w_lower.endswith("ability") and len(w_lower) > 9:
        c = w_lower[:-7] + "able"  # "ability" is 7 chars
        if len(c) >= _WORDNET_MIN_LEN:
            out.add(c)
        # dischargability -> dischargeable (stem ends in g). Edited by Cursor.
        stem7 = w_lower[:-7]
    ...
    if w_lower.endswith("ency") and len(w_lower) > 6:
        ...
        if len(c) >= _WORDNET_MIN_LEN:
            out.add(c)
    # -ization / -isation (routinization -> routinize). Edited by Cursor.
    if w_lower.endswith("ization") and len(w_lower) > 8:
        c = w_lower[:-7] + "ize"  # "ization" is 7 chars
        if len(c) >= _WORDNET_MIN_LEN:
            out.add(c)
    if w_lower.endswith("isation") and len(w_lower) > 8:
        ...
        if len(c) >= _WORDNET_MIN_LEN:
            out.add(c)
    # non-, mal-, -ance, -ary, -ize/-ise (awareness report). Edited by Cursor.
    ...
    if w_lower.endswith("ize") and len(w_lower) >= 6:
        ...
        if len(c) >= _WORDNET_MIN_LEN:
            out.add(c)
    # -ation -> -e (analyzation -> analyze). Edited by Cursor.
    if w_lower.endswith("ation") and len(w_lower) > 7:
        c = w_lower[:-5] + "e"
        if len(c) >= _WORDNET_MIN_LEN:
            out.add(c)
    # -ably -> -able (irrebuttably -> irrebuttable). Edited by Cursor.
    if w_lower.endswith("ably") and len(w_lower) > 5:
        c = w_lower[:-1] + "e"
        if len(c) >= _WORDNET_MIN_LEN:
            out.add(c)
    # -atize -> -ate (illegitimatize -> illegitimate). Edited by Cursor.
...
def _wordnet_check(w: str) -> bool:
    """Return True if w is in WordNet (synsets or morphy base form). Edited by Cursor."""
    try:
        wn = _get_wordnet()
...
class _CascadeChecker:
    """Check word via cascade: enchant dicts, WordNet, then legal dict. Edited by Cursor."""
                ...
                for c in _word_candidates(w):
                    if c != w:
                        ...
                        if self._use_wordnet and _wordnet_check(c):
                            result = True
                            break
            # Possessive: accept when stripped form is valid (e.g. taxpayers' -> taxpayers). Edited by Cursor.
...
def is_valid_word(
    ...
    """Return True if word is valid: in checker or (for frozenset) any stem/candidate in checker. Edited by Cursor."""
...
# Single letters valid for rule logic (split_word_merge, repeated_word_accept, etc.). Edited by Cursor.
...
def is_valid_word_for_rules(
    ...
    """Return False if word is a single lowercase letter not in allowlist; else is_valid_word. Edited by Cursor."""
    w = word.lower()
...
def _enchant_providers_for_tag(tag: str) -> list[str]:
    """Read enchant.ordering and return provider names for tag in cascade order. Edited by Cursor."""
    for path in (
...
def _build_enchant_dicts_for_tag(tag: str) -> list:
    """Build list of enchant Dict for one language tag (one per provider), cascade order. Edited by Cursor."""
...
def _build_enchant_dicts() -> list:
    """Build list of enchant Dict: en_US first, then la (Latin) if available. Edited by Cursor."""
...
def get_english_dictionary() -> frozenset[str] | _EnchantChecker | _CascadeChecker:
    ...
    Cascade order: (1) enchant en_US, (2) enchant la (Latin, if installed), (3) WordNet,
